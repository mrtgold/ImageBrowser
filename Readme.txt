...
* Add caching thumbnails to file on disk.
* Separate directory enumerating/thumbnail generation into a crawler that would run on the file server and pre-build cached images. Client would load from cached thumbnail files.
...
* 99% code coverage != 99% tested. Especially in a multi-threaded app, there are bound to be race conditions that are not mitigated by TDD or complete code coverage. 